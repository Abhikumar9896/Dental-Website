...
export default function ServicesGridSection() {
  return (
    <div className="absolute left-0 top-[6351px] w-full flex flex-col items-center bg-[#EDEDED] pt-16">
      <Stagger className="w-[1320px] grid grid-cols-4 gap-x-10 gap-y-14" gap={0.08}>
        {services.map((service, index) => (
          <StaggerItem key={index} y={28} className="flex flex-col gap-4 group">
            <div className="flex gap-3 items-start">
              ...
              <div className="flex flex-col gap-1.5">
                <h3 className="font-poppins text-[17px] font-semibold text-[#28231F]">
                  {service.title}
                </h3>
                <p className="font-poppins text-[#777] text-[13px] leading-[1.6]">
                  {service.description}
                </p>
              </div>
            </div>
      ...
      <Reveal y={30} className="mt-16 flex justify-center mb-[-27px]">
        <Link
          to="/services"
          className="flex h-[54px] items-center justify-center rounded-md bg-[#C187A4] px-8 font-poppins font-medium text-white hover:opacity-90 transition-opacity shadow-[0_4px_14px_rgba(193,135,164,0.4)] text-base"
        >
